...
//    getRides();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: promoList.length > 0
          ? InkWell(
              onTap: () {
                              ...
                              child: Row(
                                ...
                                children: [
                                  Image.asset(
                                    rideList[index].catType == "Bike"
                                        ? "assets/cars/car1.png"
                                        : rideList[index].catType.toLowerCase() == "auto"?  "assets/cars/auto.png" : "assets/cars/car2.png",
                                    height: 40,
                                    width: 40,
                                  ),
                                  SizedBox(
                                  ...
                                  Column(
                                    ...
                                    children: [
                                      Text(
                                        rideList[index].catType == "Bike"
                                            ? "${getTranslated(context, "GO")!} Bike"
                                            :rideList[index].catType.toLowerCase() == "auto"? "${getTranslated(context, "GO")!} Auto": getTranslated(context, "GO")! + ' ' +
              ...
              child: Row(
                ...
                children: [
                  InkWell(
                    ...
                    child: Container(
                      width: 75.w,
                      height: bookingDate != null && bookingDate!.minute > DateTime.now().minute
                          ? 7.h
                          : 6.h,
                      decoration: boxDecoration(
                          radius: 10, bgColor: Colors.red),
                      child: Center(
                          child: saveStatus
                              ? text(
                                  bookingDate != null
                      ...
                      Container(
                        ...
                        child: Image.asset(
                          rideList[_currentCar].catType != "" &&
                                  rideList[_currentCar].catType != "Bike"
                              ? "assets/cars/car2.png"
                              : "assets/cars/car1.png",
                          height: 30,
                          width: 30,
  ...
  String getTypeValue(int currentCar, List<RideModel> rideList,bool isTrueCar) {
    if (rideList[currentCar].cartype == "Auto") {
      return isTrueCar ? "Auto" : "3";
    }
    else if (rideList[currentCar].cartype != "" ) {
      return isTrueCar ? "Car" : "2";
    } else {
      return isTrueCar ? "Bike" : "1";
    }
  }
  ...
  addRides() async {
    try {
      ...
      Map params = {
        ...
        "taxi_type": getTypeValue(_currentCar, rideList, true),
        // rideList[_currentCar].cartype != ""  rideList[_currentCar].cartype : "Bike",
        "cancel_charge": rideList[_currentCar].cancellation_charges,
        "delivery_type": getTypeValue(_currentCar, rideList, false),
        // rideList[_currentCar].cartype != "" && rideList[_currentCar].cartype != "Bike" ? "2" : "1" && rideList[_currentCar].cartype != "Auto" ? "3" : "1",
  ...
  addScheduleRides() async {
    try {
      ...
      print("add schedule rideee $params");
      Map response = await apiBase.postAPICall(
  ...
  addInterCityRides() async {
    try {
      ...
      print("add intercity rideeeee $params");
      Map response = await apiBase.postAPICall(
  ...
  getRides(totalTime) async {
    try {
      ...
      if (response['status']) {
        ...
        getPromo();
        print("gettttt ride list responsee ${rideList}");
        if (rideList[_currentCar].surge_charge != null &&
            rideList[_currentCar].surge_charge.length > 0 &&
            rideList[_currentCar].surge_charge[0]['time_on_off'].toString() != "CLOSED") {
          surge = ((double.parse(rideList[_currentCar]
                          .surge_charge[0]['amount']
                          .toString()) *
                      double.parse(rideList[_currentCar].intailrate)) /
  ...
  getTime1(lat1, lon1, lat2, lon2) async {
    if (lat1 != "" &&
      ...
      http.Response response = await http.get(Uri.parse(
          "https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=${lat1},${lon1}&destinations=${lat2},${lon2}&key=AIzaSyAJnPtake2XzMTRyeiSTOl3Osw-92DwOJs"));
...
/*  applyCode(code) async {
    try {
      ...
      gst += double.parse(rideList[_currentCar].intailrate).roundToDouble();
      Map params = {
        "final_total": gst.toString(),
        "promo_code": code,
        "user_id": curUserId,
